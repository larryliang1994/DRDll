...
void mouseHandler(int event, int x, int y, int, void*)
{
    ...
    if (event == EVENT_RBUTTONUP)
    {
        ...
        Mat newImage, newMask;
        Rect boundingBox;
        Preproccessor::createSourceMask(img0, res1, 50, newImage, newMask, boundingBox);

        //imwrite("mask3.jpg", newImage);
        //imwrite("mask4.jpg", newMask);
        
        for (int i = 0; i < res1.rows; i++)
        {
            for (int j = 0; j < res1.cols; j++)
            {
                if ((int)res1.at<uchar>(i, j) == 0)
                {
                    res1.at<uchar>(i, j) = 255;
                }
                else
                {
                    res1.at<uchar>(i, j) = 0;
